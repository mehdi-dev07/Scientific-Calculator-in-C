...
inputNode *create_inputnode(char data)
{
...
bool if_empty_queue(Queue *queue)
{
...
//====================functions to check before=========================================//
bool checkforValidity(inputList *l)
{
    Queue *Q = create_queue();
    bool absError = true;
    int pCounter = 0;

    /*here we'll fill our queue with all
      the the elements while checking for order mistakes*/
    ...
    if (curr != NULL)
    {
        if (check_if_operator(curr->data) && curr->data != '-')
        {
            free_queue(Q);
            return false;
        }
    ...
    while (curr != NULL)
    {
        ...
        if (curr->data == '(' && curr->next != NULL && check_if_operator(curr->next->data))
        {
            if (curr->next->data == '-')
            {
                /* all good */
        ...
        if (check_if_operator(curr->data) && curr->next != NULL && curr->next->data == ')')
        {
            free_queue(Q);
            return false;
        }

        if (curr->data >= '0' && curr->data <= '9' && curr->next != NULL && curr->next->data == '(')
        {
            curr2 = curr->next;
            curr->next = create_inputnode('*');
            curr->next->next = curr2;
        }

        if (check_if_operator(curr->data) && curr->next != NULL && check_if_operator(curr->next->data))
        {
            free_queue(Q);
            return false;
        }
        ...
        {
            curr2 = curr->next;
            curr->next = create_inputnode('0');
            curr->next->next = curr2;
        }

        if ((check_if_operator(curr->data) && curr->data == '(') || curr->data == ')' || curr->data == '|')
        {
            decCheck = false;
        }
        else if (curr->data == '.')
    ...
    while (!if_empty_queue(Q) && pCounter >= 0)
    {
        if (peek_queue(Q) == '(' && opCheck == false)
        {
            pCounter++;
        }
        else if (peek_queue(Q) == ')' && opCheck == false)
        {
            pCounter--;
        }
        else if (peek_queue(Q) == '|')
        {
            opCheck = !opCheck;
        }
        else
...
// 🧹 CLEAR LE TERMINAL AU DÉBUT
    ...
    while (1)
    {
        ...
        if (!checkforValidity(inp_list))
        {
            printf("\n❌ Please type your expression correctly\n\n");
        ...
        printf("└─────────────────────────────────────────────────────────────┘\n");
        if (time_taken < 1.0)
            printf("  %.3f ms (< 0.001 seconds)\n", time_taken);
        else
            printf("  %.3f ms (%.6f seconds)\n", time_taken, time_taken / 1000.0);
...
/* Basic Functions (Factorial, Fibonacci),
Matrix Operations (Determinants 2x2, 3x3),
Complex Number Arithmetic,
Polynomial Solver (Quadratic Equations),
Probability Distributions,
Expression Evaluation (Infix to Postfix),
Number Theory (Prime, GCD, Factorization),
